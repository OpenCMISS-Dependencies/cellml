...
IF(Redland_CONFIG AND Redland_RAPTOR_CONFIG AND Redland_RASQAL_CONFIG)
  ...
  SET(Redland_DEFINITIONS "${Redland_CFLAGS} ${Redland_RAPTOR_CFLAGS} ${Redland_RASQAL_CFLAGS}")
  ...
  SET(Redland_LIBRARIES "${Redland_LIBS} ${Redland_RAPTOR_LIBS} ${Redland_RASQAL_LIBS}")

  EXECUTE_PROCESS(COMMAND ${Redland_CONFIG} --version OUTPUT_VARIABLE Redland_VERSION OUTPUT_STRIP_TRAILING_WHITESPACE)
ELSE(Redland_CONFIG AND Redland_RAPTOR_CONFIG AND Redland_RASQAL_CONFIG)
    FIND_PATH(REDLAND_INCLUDE_DIR librdf.h)
    SET(Redland_DEFINITIONS "")
    INCLUDE_DIRECTORIES("${REDLAND_INCLUDE_DIR}")
    FIND_LIBRARY(Redland_LIBS librdf)
    FIND_LIBRARY(Redland_RAPTOR_LIBS raptor)
    FIND_LIBRARY(Redland_RASQAL_LIBS rasqal)
    SET(Redland_LIBRARIES ${Redland_LIBS} ${Redland_RASQAL_LIBS} ${Redland_RAPTOR_LIBS})
    SET(Redland_VERSION "1.0.9")
...
IF (Redland_FOUND)
   IF (NOT Redland_FIND_QUIETLY)
      MESSAGE(STATUS "Found Redland (${Redland_VERSION}): ${Redland_DEFINITIONS}")
